...
class MultiHeadDotProductAttention(hk.Module):
  ...
  def __call__(
  ...
  ) -> tuple[jax.Array, jax.Array]:  # Updated to return attention weights
    """Returns the output of the multi-head attention and attention weights."""
    ...
    return output, normalized_attention  # Return both output and attention weights
...
def _attention_block(inputs: jax.Array, config: TransformerConfig) -> tuple[jax.Array, jax.Array]:
...
def transformer_decoder(
  ...
  """Returns the transformer decoder output and attention weights.
  ...
  Returns:
    A tuple containing:
      - Logits: The log-softmax logits, shape [B, T, V].
      - Attention weights: A list of attention weight tensors from each layer.
  """
  ...
  for _ in range(config.num_layers):
    ...
    attention_output, attn_weights = _attention_block(attention_input, config)
...
def build_transformer_predictor(
  ...
  """Returns a transformer predictor with attention weights."""
